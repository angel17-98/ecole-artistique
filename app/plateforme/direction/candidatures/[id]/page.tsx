...
export default async function CandidatureDetailPage({
  ...
  // Groupes actifs disponibles pour assignation
  const { data: groupes } = await supabase
    .from("groupes")
    .select("*, parcours(nom, type), salle:salles(nom)")
    .eq("actif", true)
    .order("created_at", { ascending: true });

  // Notes direction sur cette candidature
  const { data: notesDiriData } = await supabase
    .from("notes_direction")
    .select("id, contenu, created_at, auteur:profiles!auteur_id(prenom, nom)")
  ...
  // Rang dans les candidatures (ordre d'arrivée pour ce parcours)
