...
//       // On log mais on ne throw pas — l'action doit continuer même si l'email échoue
//     }
//     return result;
//   } catch (err) {
...
// // ── Helper : log automatique dans notes_direction ─────────────────────────────
...
//     const registerUrl = `${process.env.NEXT_PUBLIC_SITE_URL}/plateforme/register?email=${encodeURIComponent(candidature.email)}&source=candidature`;

//     // ── Email différent selon si le candidat a un compte ou non ──────────────
//     if (existingUser) {
//       // A un compte → email sans CTA de création
//       await sendEmail({
...
//                 <p style="font-size: 13px; color: rgb(22,92,71); font-weight: 600; margin: 0 0 6px;">📱 Suis ta candidature</p>
//                 <p style="font-size: 13px; color: rgba(0,0,0,0.6); margin: 0; line-height: 1.6;">
//                   Connecte-toi à ton espace Crea'Star pour suivre l'avancement en temps réel.
//                 </p>
//                 <a href="${process.env.NEXT_PUBLIC_SITE_URL}/plateforme/dashboard"
//                   style="display: inline-block; margin-top: 12px; background: rgb(22,92,71); color: white; padding: 10px 20px; border-radius: 100px; text-decoration: none; font-size: 13px; font-weight: 600;">
//                   Mon espace →
//                 </a>
//               </div>
...
//         "Candidature retenue 🎉",
//         `Ta candidature pour le parcours ${parcoursLabel} a été retenue !`,
//         "/plateforme/dashboard");
//       await logAction(id, user.id, "systeme", "📱 Notification in-app envoyée au candidat");

//     } else {
//       // Pas de compte → email avec CTA de création de compte
//       await sendEmail({
//         from: process.env.RESEND_FROM_EMAIL!,
//         to: candidature.email,
//         subject: `Ta candidature Crea'Star est retenue 🎉`,
...
//               <!-- CTA création de compte — uniquement pour les candidats sans compte -->
//               <div style="background: rgb(239,244,239); border-radius: 12px; padding: 20px; margin: 20px 0; border: 1px solid rgba(22,92,71,0.15);">
//                 <p style="font-size: 13px; color: rgb(22,92,71); font-weight: 600; margin: 0 0 8px;">
...
//         "📧 Email d'acceptation envoyé avec invitation à créer un compte");
//     }

//     await logAction(id, user.id, "action",
//       `✅ Candidature marquée acceptable par ${auteurNom}`);
...
//           </div>
//           <div style="background: white; padding: 32px; border: 1px solid rgba(0,0,0,0.06); border-radius: 0 0 16px 16px;">
//             <p style="font-size: 15px; color: #111; line-height: 1.7;">Ta candidature pour le parcours <strong>${parcoursLabel}</strong> a bien été reçue et étudiée.</p>
...
//     await logAction(id, user.id, "action",
//       `🕐 Mis en liste d'attente par ${auteurNom}`);
...
//     // Email sauvegardé dans les notes avec type "email"
//     await supabaseAdmin.from("notes_direction").insert({
//       candidature_id: id,
//       auteur_id: user.id,
//       type: "email",
...
//     await logAction(id, user.id, "action",
//       `📬 Demande d'info envoyée par ${auteurNom}`);
...
//     await logAction(id, user.id, "action",
//       `🎯 Place proposée par ${auteurNom} — délai ${delaiJours}j (expire le ${new Date(expireAt).toLocaleDateString("fr-BE", { day: "numeric", month: "long" })})`);
...
//     const { error: noteError } = await supabaseAdmin.from("notes_direction").insert({
//       candidature_id: id,
//       auteur_id: user.id,
//       type: "note",
...
//     await supabaseAdmin.from("notes_direction")
//       .delete().eq("id", body.noteId).eq("auteur_id", user.id);
//     return NextResponse.json({ success: true, action: "supprimer_note" });
//   }

//   // ── ACTION : RETIRER DU GROUPE → retour zone tampon ──────────────────────────
//   if (action === "retirer_groupe") {
//     await supabaseAdmin.from("candidatures").update({
//       groupe_inscription_id: null,
//       statut: "validee",
...
//     await logAction(id, user.id, "action",
//       `↩️ Retiré du groupe par ${auteurNom} — retour en zone tampon`);

//     return NextResponse.json({ success: true, action: "retirer_groupe" });
//   }
