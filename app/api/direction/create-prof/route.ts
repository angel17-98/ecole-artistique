...
export async function POST(req: Request) {
  ...
  const {
    ...
    salaireFixe, tarifCoursIndiv, avantages, heuresMinPeriode, periodeEngagement, dateDebut,
  } = await req.json();
  ...
  // Un prof n'existe jamais sans contrat actif : on valide les champs du contrat
  // dès la création plutôt que de laisser un compte "orphelin" en attente.
  if (typeContrat === "salarie" && !salaireFixe) {
    return NextResponse.json({ error: "Le salaire fixe est obligatoire pour un contrat salarié." }, { status: 400 });
  }
  if (typeContrat === "independant" && !tarifCoursIndiv) {
    return NextResponse.json({ error: "Le tarif par cours est obligatoire pour un contrat indépendant." }, { status: 400 });
  }
  ...
  if (profError || !profData) {
    console.error("Erreur profs:", profError);
    await supabaseAdmin.auth.admin.deleteUser(newUserId);
    return NextResponse.json({ error: profError?.message ?? "Erreur création du profil professeur" }, { status: 500 });
  }

  // ── 6bis. Insérer le contrat — un prof n'existe jamais sans contrat actif ──
  const { error: contratError } = await supabaseAdmin
    .from("contrats")
    .insert({
      prof_id: profData.id,
      type: typeContrat,
      salaire_fixe: salaireFixe ? parseFloat(salaireFixe) : null,
      tarif_cours_indiv: tarifCoursIndiv ? parseFloat(tarifCoursIndiv) : null,
      avantages: avantages || null,
      heures_min_periode: heuresMinPeriode ? parseInt(heuresMinPeriode) : null,
      periode_engagement: periodeEngagement || null,
      date_debut: dateDebut || new Date().toISOString().split("T")[0],
    });

  if (contratError) {
    console.error("Erreur contrats:", contratError);
    // Rollback complet — un prof créé sans contrat validerait le problème qu'on corrige
    await supabaseAdmin.from("profs").delete().eq("id", profData.id);
    await supabaseAdmin.from("profiles").delete().eq("id", newUserId);
    await supabaseAdmin.auth.admin.deleteUser(newUserId);
    return NextResponse.json({ error: `Erreur création du contrat : ${contratError.message}` }, { status: 500 });
  }
