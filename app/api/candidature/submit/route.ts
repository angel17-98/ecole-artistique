            ...
            <div style="background: #fffbf5; padding: 24px 28px; border: 1px solid rgba(0,0,0,0.06); border-radius: 0 0 12px 12px;">
              ...
              <div style="background: rgb(255,245,230); border-radius: 10px; padding: 14px 18px; margin-bottom: 20px;">
                <p style="font-size: 13px; color: rgb(120,60,0); margin: 0; line-height: 1.6;">
                  <strong>Action recommandée :</strong> Supprime les vidéos des candidatures déjà examinées dans Supabase Storage → bucket <code>candidature-videos</code>.
                </p>
              </div>
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { error: dbError } = await supabase.from("candidatures").insert({
      ...
      eval_chant, eval_danse, eval_theatre,
      eval_ecriture, eval_scenique, eval_studio,
      video_url: video_url || null,           // lien YouTube/Drive manuel
      video_link: video_link || null,         // lien saisi manuellement
      drive_video_url: drive_video_url || null, // fichier uploadé sur Drive
      drive_file_id: drive_file_id || null,   // ID Drive pour suppression éventuelle
      statut: "en_attente",
    ...
    const videoBlock = (() => {
      const parts = [];
      if (video_url) parts.push(`<a href="${video_url}" style="display: inline-block; background: rgb(22,92,71); color: white; text-decoration: none; padding: 10px 20px; border-radius: 100px; font-size: 13px; font-weight: 600; margin-right: 8px; margin-bottom: 8px;">▶ Voir le fichier vidéo</a>`);
      if (video_link) parts.push(`<a href="${video_link}" style="display: inline-block; background: rgb(12,50,38); color: white; text-decoration: none; padding: 10px 20px; border-radius: 100px; font-size: 13px; font-weight: 600; margin-bottom: 8px;">▶ Voir la vidéo en ligne</a><p style="font-size: 12px; color: rgba(0,0,0,0.4); margin: 4px 0 0;">${video_link}</p>`);
      return parts.length > 0 ? `<hr style="border: none; border-top: 1px solid rgba(0,0,0,0.08); margin: 20px 0;">${parts.join("")}` : "";
    })();

    // ── 4. Email confirmation candidat — avec les deux CTA ────────────────────
        ...
        <div style="font-family: sans-serif; max-width: 560px; margin: 0 auto; color: #111;">
          ...
          <div style="background: #f9f8f5; padding: 28px 32px; border-radius: 0 0 16px 16px; border: 1px solid rgba(0,0,0,0.06);">
            <p style="font-size: 15px; line-height: 1.7; margin: 0 0 16px;">
              ...
              On prend le temps de lire chaque dossier attentivement — tu recevras notre réponse <strong>dans la semaine</strong>.
            </p>

            <!-- CTA principal : créer un compte -->
            <div style="background: rgb(239,244,239); border-radius: 14px; padding: 20px 24px; margin: 24px 0; border: 1px solid rgba(22,92,71,0.15);">
              <p style="font-size: 14px; font-weight: 600; color: rgb(12,50,38); margin: 0 0 6px;">
                📱 Suis ta candidature en temps réel
              </p>
              <p style="font-size: 13px; color: rgba(0,0,0,0.6); line-height: 1.6; margin: 0 0 16px;">
                Crée ton espace Crea'Star pour voir l'état de ta candidature, recevoir les notifications importantes et t'inscrire dès qu'une place est disponible.
              </p>
              <a href="${registerUrl}"
                style="display: inline-block; background: rgb(22,92,71); color: white; text-decoration: none; padding: 12px 24px; border-radius: 100px; font-size: 13px; font-weight: 600;">
                Créer mon espace →
              </a>
            </div>

            <p style="font-size: 13px; color: rgba(0,0,0,0.45); margin: 0 0 4px;">
              Tu préfères attendre notre réponse par email ? Pas de problème — on te tient au courant quoi qu'il arrive.
            </p>
